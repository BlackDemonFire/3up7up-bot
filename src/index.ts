...
rl.on("line", async (line) => {
	if (line.toLowerCase() !== "stop") return;
...
discordClient.on("messageCreate", (message) => {
	...
	if (
		...
		if (message.content.startsWith("!set ")) return setValue(message);
		if (message.content.startsWith("!setgame")) return setGame(message);
	}
	//game check
	if (!discordClient.counter.has(message.channel.id)) return;
